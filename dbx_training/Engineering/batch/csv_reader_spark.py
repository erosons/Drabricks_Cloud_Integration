...
schema = StructType([
    ...
    StructField("student_phone_numbers", ArrayType(
        StringType(), containsNull=True), nullable=True),
    StructField("student_address", MapType(
...
def student_1():
    ...
    file_path = os.path.expanduser(
        '/home/samson/Desktop/my_sandbox_new/Databricks-project/sample-case_1.csv')
...
def student_append():
    ...
    """
    file_path = os.path.expanduser(
        '/home/samson/Desktop/my_sandbox_new/Databricks-project/sample-case_5.csv')
    df = pd.read_csv(file_path)
    d = df.to_dict(orient='records')
    df = spark.createDataFrame(d)
    df = df.withColumn("record_hash", sha2(concat_ws("||", *df.columns), 256))
