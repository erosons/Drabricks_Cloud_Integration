...
class ResolvedTable:
    ...
    mnpi_expiration_date: Optional[str] = None
...
class ABACConfigLoader:
    ...
    def load(self) -> GovernanceManifest:
        ...
        # 2. Load templates
        templates_path = os.path.join(self.securables_path, "_templates_tags.yaml")
    ...
    def _resolve_table_file(self, filepath: str) -> Optional[ResolvedTable]:
        ...
        # Merge grants: auto_discover defaults + template defaults + table-specific
        auto_discover_grants = self._get_auto_discover_grants(
            raw.get("catalog", ""), raw.get("schema", "")
        )
        merged_grants = self._merge_grants(
            template, raw.get("grants", []), auto_discover_grants
        )
    ...
    def _get_auto_discover_grants(
        self, catalog: str, schema: str
    ) -> List[Dict[str, Any]]:
        """Get default_grants from the matching auto_discover rule."""
        for rule in self._defaults.get("auto_discover", []):
            if rule.get("catalog") == catalog and rule.get("schema") == schema:
                return rule.get("default_grants", [])
        return []

    def _merge_grants(
        self, template: Dict[str, Any], table_grants: List[Dict[str, Any]],
        auto_discover_grants: Optional[List[Dict[str, Any]]] = None
    ) -> List[Dict[str, Any]]:
        """Merge grants with priority: table-specific > auto_discover defaults > template defaults."""
        # Table-specific grants override all defaults for same group
        table_groups = {g.get("group", g.get("role", "")) for g in table_grants}

        merged = []

        # 1. Add auto_discover default_grants that aren't overridden by table
        for grant in (auto_discover_grants or []):
            group = grant.get("group", grant.get("role", ""))
            if group not in table_groups:
                merged.append(grant)

        # 2. Add template defaults that aren't overridden by table or auto_discover
        merged_groups = table_groups | {g.get("group", g.get("role", "")) for g in merged}
        for grant in template.get("default_grants", []):
            role = grant.get("role", "")
            if role not in merged_groups:
                merged.append(grant)

        # 3. Add all table-specific grants (highest priority)
