...
#### 3. Storage Credential IAM Policy for Directory Listing Mode (AWS)

When using **directory listing mode** (the default), the IAM role only needs S3 read and list permissions — no SQS/SNS infrastructure is required:
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:s3:::prod-landing-bucket/orders/*"
    },
    {
      "Sid": "SelfAssumeRole",
      "Effect": "Allow",
...
**Trust Relationship Policy** (allows Unity Catalog to assume this role):
...
> **Note:** Directory listing mode does NOT require `s3:PutObject`, `s3:DeleteObject`, SQS, or SNS permissions. The `s3:ListBucket` with a prefix condition scopes listing to only the relevant directory path, following least-privilege principles.

#### 4. AutoLoader Options for File Notification (AWS)
