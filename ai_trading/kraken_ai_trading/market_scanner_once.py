"""
Pair screener + mover scanner — single run, designed for cron.
...
Output sections:
  1. ELIGIBLE KRAKEN PAIRS   — qualified coins available on Kraken
  2. ELIGIBLE (not on Kraken) — qualified coins on Binance/Coinbase only
  3. TOP MOVERS              — 10–50% 24h moves (momentum alerts)
  4. CONFIGURED PAIRS FAILING — env files whose coins dropped below thresholds

Cron entry (EC2):
  */5 * * * * cd /opt/kraken_bot && python3 market_scanner_once.py >> logs/scanner.log 2>&1
"""
...
CMC_PAGE_SIZE    = 500            # top N coins by market cap
MIN_MARKET_CAP   = 300_000_000   # $300M
MIN_VOL_24H      = 1_000_000     # $1M daily volume
MIN_VOL_MC_RATIO = 5.0           # 5% volume-to-market-cap
MOVE_LOW_PCT     = 10.0          # mover: min 24h move
MOVE_HIGH_PCT    = 50.0          # mover: max 24h move (above = likely pump/rug)
OTHER_ELIGIBLE_CAP = 25          # max non-Kraken eligible coins to display
...
def _print_eligible(coins: List[Coin]) -> None:
    ...
    log.info("─" * 95)
    log.info("ELIGIBLE KRAKEN PAIRS  (MC≥$300M | Vol24h≥$1M | V/MC≥5%% | on Binance/CB/Kraken)  — %d", len(kraken_eligible))
    log.info("─" * 95)
    if kraken_eligible:
    ...
    if other_eligible:
        log.info("")
        log.info("ELIGIBLE — not on Kraken yet  (%d coins, top %d shown)  [watch for Kraken listing]",
...
def _print_failing(coins: List[Coin]) -> None:
    ...
    for c in failing:
        ...
        if not c.on_any:
            reasons.append("not listed on any exchange")
...
def main() -> None:
    ...
    log.info("")
    log.info("━" * 95)
    log.info("  PAIR SCREENER  %s  |  MC≥$300M  Vol≥$1M  V/MC≥5%%  |  CMC top %d",
             ts, CMC_PAGE_SIZE)
    log.info("━" * 95)

    log.info("Fetching exchange listings ...")
    kraken    = _kraken_symbols()
    binance   = _binance_symbols()
    coinbase  = _coinbase_symbols()
    configured = _configured_symbols()
    log.info("Kraken=%d  Binance=%d  Coinbase=%d  Configured=%d",
             len(kraken), len(binance), len(coinbase), len(configured))

    log.info("Fetching CMC top %d by market cap ...", CMC_PAGE_SIZE)
    coins = _fetch_coins(kraken, binance, coinbase, configured)
    if not coins:
        log.warning("No CMC data returned — skipping scan")
        return
    log.info("Coins fetched: %d", len(coins))
